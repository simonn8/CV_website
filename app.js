/* ==========================================================================
   Simon Nachtergaele — Portfolio, Sound Stream & Library Architecture
   ========================================================================== */

// Embedded 47-Book Dataset from Goodreads
...
async function initLibraryCatalog() {
  ...
  const syncBadge = document.getElementById('db-sync-status-badge');
  ...
  if (syncBtn) {
    syncBtn.addEventListener('click', async () => {
      ...
      if (syncBadge) syncBadge.textContent = 'Updating...';
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (json.items && json.items.length > 0) {
            if (syncBadge) syncBadge.textContent = `Synced (${json.items.length} live)`;
          }
        }
      } catch (e) {
...
function renderLibrary() {
  ...
  container.innerHTML = filtered.map((book) => {
    ...
    const stars = renderStars(book.user_rating, isReading);
    ...
    return `
      <article class="book-entry">
        <div class="book-cover-wrap">
          <img src="${cover}" alt="${escapeHtml(book.title)}" class="book-img" loading="lazy" />
          <span class="shelf-tag ${shelfClass}">${shelfLabel}</span>
        </div>
        <div class="book-details">
          <div>
            <h4 class="book-title">${escapeHtml(book.title)}</h4>
            <p class="book-author">${escapeHtml(book.author)} ${year}</p>
            <div class="book-rating">${stars}</div>
            <p class="book-synopsis">${escapeHtml(book.synopsis || 'No overview logged.')}</p>
          </div>
          <a href="${book.goodreads_url || '#'}" target="_blank" rel="noopener noreferrer" class="goodreads-btn">
            View on Goodreads ↗
          </a>
        </div>
      </article>
...
function renderStars(rating, isReading) {
  ...
  if (r === 0) return '<span style="color: var(--text-muted); font-size: 11px;">Unrated</span>';
  const full = '★'.repeat(r);
